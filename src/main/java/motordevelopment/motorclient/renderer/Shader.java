...
public class Shader {
    ...
    public Shader(String vertPath, String fragPath) {
        ...
        if (vertError != null) {
            MotorClient.LOG.error("Failed to compile vertex shader ({}): {}", vertPath, vertError);
        ...
        if (fragError != null) {
            MotorClient.LOG.error("Failed to compile fragment shader ({}): {}", fragPath, fragError);
        ...
        if (programError != null) {
            MotorClient.LOG.error("Failed to link program: {}", programError);
    ...
    private String read(String path) {
        try {
            return IOUtils.toString(mc.getResourceManager().getResource(MotorClient.identifier("shaders/" + path)).get().getInputStream(), StandardCharsets.UTF_8);
        } catch (IOException e) {
