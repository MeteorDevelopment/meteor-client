...
public class MotorTextHud {
    public static final HudElementInfo<TextHud> INFO = new HudElementInfo<>(Hud.GROUP, "text", "Displays arbitrary text with Starscript.", MotorTextHud::create);
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "Kill Aura: {motor.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "Kill Aura: {motor.is_module_active(\"kill-aura\") ? #2 \"ON\" : #3 \"OFF\"} #1{motor.get_module_info(\"kill-aura\")}", 0);
        WATERMARK = addPreset("Watermark", "{motor.name} #1{motor.version}");
