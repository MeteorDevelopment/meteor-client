...
import motordevelopment.motorclient.systems.hud.elements.*;
...
public class Hud extends System<Hud> implements Iterable<HudElement> {
    public static final HudGroup GROUP = new HudGroup("motor");
    ...
    private final Setting<Boolean> hideInMenus = sgGeneral.add(new BoolSetting.Builder()
        .name("hide-in-menus")
        .description("Hides the motor hud when in inventory screens or game menus.")
        .defaultValue(false)
        .build()
    );
    ...
    private void resetToDefaultElementsImpl() {
        ...
        // Top Left
        add(MotorTextHud.WATERMARK, 4, 4, XAnchor.Left, YAnchor.Top);
        add(MotorTextHud.FPS, 4, 4 + h, XAnchor.Left, YAnchor.Top);
        add(MotorTextHud.TPS, 4, 4 + h * 2, XAnchor.Left, YAnchor.Top);
        add(MotorTextHud.PING, 4, 4 + h * 3, XAnchor.Left, YAnchor.Top);
        add(MotorTextHud.SPEED, 4, 4 + h * 4, XAnchor.Left, YAnchor.Top);

        // Top Right
        add(ActiveModulesHud.INFO, -4, 4, XAnchor.Right, YAnchor.Top);

        // Bottom Right
        add(MotorTextHud.POSITION, -4, -4, XAnchor.Right, YAnchor.Bottom);
        add(MotorTextHud.OPPOSITE_POSITION, -4, -4 - h, XAnchor.Right, YAnchor.Bottom);
        add(MotorTextHud.ROTATION, -4, -4 - h * 2, XAnchor.Right, YAnchor.Bottom);
    }
