...
public class StashFinder extends Module {
    ...
    public StashFinder() {
        super(Categories.World, "stash-finder", "Searches loaded chunks for storage blocks. Saves to <your minecraft folder>/motor-client");
    }
        ...
        if (chunk.getTotal() >= minimumStorageCount.get()) {
            ...
            if (sendNotifications.get() && (!chunk.equals(prevChunk) || !chunk.countsEqual(prevChunk))) {
                switch (notificationMode.get()) {
                    ...
                    case Toast -> mc.getToastManager().add(new MotorToast(Items.CHEST, title, "Found Stash!"));
                    case Both -> {
                        info("Found stash at (highlight)%s(default), (highlight)%s(default).", chunk.x, chunk.z);
                        mc.getToastManager().add(new MotorToast(Items.CHEST, title, "Found Stash!"));
                    }
                }
            }
        }
    ...
    private File getJsonFile() {
        return new File(new File(new File(MotorClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.json");
    }

    private File getCsvFile() {
        return new File(new File(new File(MotorClient.FOLDER, "stashes"), Utils.getFileWorldName()), "stashes.csv");
    }
