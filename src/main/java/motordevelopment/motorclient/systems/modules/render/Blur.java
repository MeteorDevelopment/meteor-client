...
import motordevelopment.motorclient.renderer.*;
...
public class Blur extends Module {
    ...
    private final Setting<Boolean> motor = sgScreens.add(new BoolSetting.Builder()
        .name("motor")
        .description("Applies blur to motor screens.")
        .defaultValue(true)
        .build());
    ...
    public Blur() {
        ...
        MotorClient.EVENT_BUS.subscribe(new ConsumerListener<>(ResolutionChangedEvent.class, event -> {
            // Resize all fbos
        ...
        MotorClient.EVENT_BUS.subscribe(new ConsumerListener<>(RenderAfterWorldEvent.class, event -> onRenderAfterWorld()));
    }
    ...
    private boolean shouldRender() {
        ...
        if (screen instanceof WidgetScreen) return motor.get();
