            ...
            for (BakedQuad _quad : model.getQuads(null, face, random)) {
                ...
                for (int i = 0; i < 4; i++) {
                    switch (_quad.getFace()) {
                        case DOWN -> minY = Math.min(minY, quad.motor$getY(i));
                        case UP -> maxY = Math.max(maxY, quad.motor$getY(i));
                        case NORTH -> minZ = Math.min(minZ, quad.motor$getZ(i));
                        case SOUTH -> maxZ = Math.max(maxZ, quad.motor$getZ(i));
                        case WEST -> minX = Math.min(minX, quad.motor$getX(i));
                        case EAST -> maxX = Math.max(maxX, quad.motor$getX(i));
                    }
                }
            }
        }
