...
public class DiscordPresence extends Module {
    ...
    private final Setting<List<String>> line2Strings = sgLine2.add(new StringListSetting.Builder()
        ...
        .defaultValue("motor on Crack!", "{round(server.tps, 1)} TPS", "Playing on {server.difficulty} difficulty.", "{server.player_count} Players online")
    ...
    public DiscordPresence() {
        super(Categories.Misc, "discord-presence", "Displays motor as your presence on Discord.");
    ...
    public void onActivate() {
        ...
        String largeText = "%s %s".formatted(MotorClient.NAME, MotorClient.VERSION);
        if (!MotorClient.BUILD_NUMBER.isEmpty()) largeText += " Build: " + MotorClient.BUILD_NUMBER;
        rpc.setLargeImage("motor_client", largeText);
        ...
        if (Utils.canUpdate()) {
            ...
            if (line1Ticks >= line1UpdateDelay.get() || forceUpdate) {
                if (!line1Scripts.isEmpty()) {
                    ...
                    String message = MotorStarscript.run(line1Scripts.get(i));
            ...
            if (line2Ticks >= line2UpdateDelay.get() || forceUpdate) {
                if (!line2Scripts.isEmpty()) {
                    ...
                    String message = MotorStarscript.run(line2Scripts.get(i));
            ...
            if (!lastWasInMainMenu) {
                rpc.setDetails(MotorClient.NAME + " " + (MotorClient.BUILD_NUMBER.isEmpty() ? MotorClient.VERSION : MotorClient.VERSION + " " + MotorClient.BUILD_NUMBER));
                ...
                else if (mc.currentScreen instanceof WidgetScreen) rpc.setState("Browsing motor's GUI");
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        help.action = () -> Util.getOperatingSystem().open("https://github.com/motorDevelopment/motor-client/wiki/Starscript");
