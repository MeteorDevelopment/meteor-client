...
import motordevelopment.motorclient.mixin.CloseHandledScreenC2SPacketAccessor;
import motordevelopment.motorclient.mixin.HandledScreenAccessor;
...
public class InventoryTweaks extends Module {
    ...
    public void steal(ScreenHandler handler) {
        MotorExecutor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(ScreenHandler handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        MotorExecutor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
