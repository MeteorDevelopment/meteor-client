...
import motordevelopment.motorclient.mixin.LivingEntityAccessor;
...
public class Jesus extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (bubbleColumn) {
            if (mc.options.jumpKey.isPressed() && mc.player.getVelocity().getY() < 0.11) ((IVec3d) mc.player.getVelocity()).motor$setY(0.11);
            return;
        }

        // Move up
        if (mc.player.isTouchingWater() || mc.player.isInLava()) {
            ((IVec3d) mc.player.getVelocity()).motor$setY(0.11);
            tickTimer = 0;
            return;
        }
