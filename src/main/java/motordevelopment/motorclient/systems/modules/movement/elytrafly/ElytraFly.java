...
package motordevelopment.motorclient.systems.modules.movement.elytrafly;
...
import motordevelopment.motorclient.systems.modules.movement.elytrafly.modes.Packet;
import motordevelopment.motorclient.systems.modules.movement.elytrafly.modes.Pitch40;
import motordevelopment.motorclient.systems.modules.movement.elytrafly.modes.Vanilla;
import motordevelopment.motorclient.systems.modules.player.ChestSwap;
import motordevelopment.motorclient.systems.modules.player.Rotation;
...
public class ElytraFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (mc.player.isGliding()) {
            ...
            if (dontGoIntoUnloadedChunks.get()) {
                if (mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) {
                    if (flightMode.get() != ElytraFlightModes.Bounce) ((IVec3d) event.movement).motor$set(currentMode.velX, currentMode.velY, currentMode.velZ);
                } else {
                    currentMode.zeroAcceleration();
                    ((IVec3d) event.movement).motor$set(0, currentMode.velY, 0);
                }
            } else if (flightMode.get() != ElytraFlightModes.Bounce) ((IVec3d) event.movement).motor$set(currentMode.velX, currentMode.velY, currentMode.velZ);
        ...
        if (autoHover.get() && mc.player.input.playerInput.sneak() && !Modules.get().get(Freecam.class).isActive() && mc.player.isGliding() && flightMode.get() != ElytraFlightModes.Bounce) {
            ...
            if (!underCollidable && under2Collidable) {
                ((IVec3d)event.movement).motor$set(event.movement.x, -0.1f, event.movement.z);
            ...
            if (underCollidable) {
                ((IVec3d)event.movement).motor$set(event.movement.x, -0.03f, event.movement.z);
                ...
                if (mc.player.getPos().y <= mc.player.getBlockPos().down().getY() + 1.34f) {
                    ((IVec3d)event.movement).motor$set(event.movement.x, 0, event.movement.z);
    ...
    protected void enableGroundListener() {
        MotorClient.EVENT_BUS.subscribe(staticGroundListener);
    }

    protected void disableGroundListener() {
        MotorClient.EVENT_BUS.unsubscribe(staticGroundListener);
    }
    ...
    protected void enableInstaDropListener() {
        MotorClient.EVENT_BUS.subscribe(staticInstadropListener);
    }

    protected void disableInstaDropListener() {
        MotorClient.EVENT_BUS.unsubscribe(staticInstadropListener);
    }
