...
public class Criticals extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (event.packet instanceof IPlayerInteractEntityC2SPacket packet && packet.motor$getType() == PlayerInteractEntityC2SPacket.InteractType.ATTACK) {
            if (mace.get() && mc.player.getMainHandStack().getItem() instanceof MaceItem) {
                ...
                Entity entity = packet.motor$getEntity();
                ...
                switch (mode.get()) {
                    ...
                    case Jump, MiniJump -> {
                        if (!sendPackets) {
                            ...
                            else ((IVec3d) mc.player.getVelocity()).motor$setY(0.25);
                            event.cancel();
                        }
                    }
                }
    ...
    private void sendPacket(double height) {
        ...
        ((IPlayerMoveC2SPacket) packet).motor$setTag(1337);
