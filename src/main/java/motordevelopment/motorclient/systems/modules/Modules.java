...
import motordevelopment.motorclient.systems.modules.combat.*;
import motordevelopment.motorclient.systems.modules.misc.*;
import motordevelopment.motorclient.systems.modules.misc.swarm.Swarm;
import motordevelopment.motorclient.systems.modules.movement.*;
import motordevelopment.motorclient.systems.modules.movement.elytrafly.ElytraFly;
import motordevelopment.motorclient.systems.modules.movement.speed.Speed;
import motordevelopment.motorclient.systems.modules.player.*;
import motordevelopment.motorclient.systems.modules.render.*;
import motordevelopment.motorclient.systems.modules.render.blockesp.BlockESP;
import motordevelopment.motorclient.systems.modules.render.marker.Marker;
import motordevelopment.motorclient.systems.modules.world.Timer;
import motordevelopment.motorclient.systems.modules.world.*;
import motordevelopment.motorclient.utils.Utils;
import motordevelopment.motorclient.utils.misc.Keybind;
import motordevelopment.motorclient.utils.misc.ValueComparableMap;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                MotorClient.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    /***
     * @see motordevelopment.motorclient.commands.commands.BindCommand
    ...
    private boolean onBinding(boolean isKey, int value, int modifiers) {
        ...
        MotorClient.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
        moduleToBind = null;
        ...
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    MotorClient.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    MotorClient.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
    ...
    public static class ModuleRegistry extends SimpleRegistry<Module> {
        public ModuleRegistry() {
            super(RegistryKey.ofRegistry(MotorClient.identifier("modules")), Lifecycle.stable());
        }
