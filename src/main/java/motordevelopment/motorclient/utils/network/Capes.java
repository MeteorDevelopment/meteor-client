...
public class Capes {
    private static final String CAPE_OWNERS_URL = "https://motorclient.com/api/capeowners";
    private static final String CAPES_URL = "https://motorclient.com/api/capes";
    ...
    @PreInit(dependencies = MotorExecutor.class)
    public static void init() {
        ...
        MotorExecutor.execute(() -> {
            ...
            Stream<String> lines = Http.get(CAPE_OWNERS_URL)
                .exceptionHandler(e -> MotorClient.LOG.error("Could not load capes: {}",  e.getMessage()))
                .sendLines();
            if (lines != null) {
                lines.forEach(s -> {
        ...
        MotorClient.EVENT_BUS.subscribe(Capes.class);
    }
    ...
    private static class Cape {
        ...
        public Cape(String name) {
            this.identifier = MotorClient.identifier("capes/" + COUNT++);
            this.name = name;
        }
