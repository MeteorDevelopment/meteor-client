...
public class ChatUtils {
    ...
    public static void init() {
        PREFIX = Text.empty()
            ...
            .append("[")
            .append(Text.literal("motor").setStyle(Style.EMPTY.withColor(TextColor.fromRgb(MotorClient.ADDON.color.getPacked()))))
            .append("] ");
    }

    public static Text getmotorPrefix() {
        return PREFIX;
    }

    /**
     * Registers a custom prefix to be used when calling from a class in the specified package. When null is returned from the supplier the default motor prefix is used.
     */
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Text msg) {
        ...
        ((IChatHud) mc.inGameHud.getChatHud()).motor$add(message, id);
    }
