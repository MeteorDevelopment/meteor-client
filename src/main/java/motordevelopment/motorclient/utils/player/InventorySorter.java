...
public class InventorySorter {
    ...
    private void generateActions() {
        ...
        for (Slot slot : screen.getScreenHandler().slots) {
            if (getInvPart(slot) == originInvPart) slots.add(new MySlot(((ISlot) slot).motor$getId(), slot.getStack()));
        }
    ...
    private InvPart getInvPart(Slot slot) {
        int i = ((ISlot) slot).motor$getIndex();

        if (slot.inventory instanceof PlayerInventory && (!(screen instanceof CreativeInventoryScreen) || ((ISlot) slot).motor$getId() > 8)) {
