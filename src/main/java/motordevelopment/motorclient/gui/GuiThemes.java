...
public class GuiThemes {
    private static final File FOLDER = new File(MotorClient.FOLDER, "gui");
    ...
    public static void add(GuiTheme theme) {
        for (Iterator<GuiTheme> it = themes.iterator(); it.hasNext();) {
            if (it.next().name.equals(theme.name)) {
                ...
                MotorClient.LOG.error("Theme with the name '{}' has already been added.", theme.name);
                break;
            }
        }
