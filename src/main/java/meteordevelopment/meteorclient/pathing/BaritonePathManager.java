    ...
    public BaritonePathManager() {
        ...
        for (Field field : klass.getDeclaredFields()) {
            if (field.getType() == Rotation.class) {
                try {
                    MethodHandles.Lookup lookup = MethodHandles.privateLookupIn(klass, MethodHandles.lookup()); //altoclef patch
                    rotationField = lookup.unreflectVarHandle(field);
                    break;
                } catch (IllegalAccessException e) {
                    e.printStackTrace(); // Log the full stack trace
                    throw new RuntimeException(e); // Provide context in the message
                }
            }
        }
