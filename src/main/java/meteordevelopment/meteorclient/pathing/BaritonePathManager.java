...
public class BaritonePathManager implements IPathManager {
    ...
    public void moveTo(BlockPos pos, boolean ignoreY) {
        ...
        BaritoneAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess().setGoalAndPath(new GoalBlock(pos.getX(), pos.getY(), pos.getZ()));
    }
    ...
    public void mine(Block... blocks) {
        BaritoneUtils.mineByBlockIds(BaritoneAPI.getProvider().getPrimaryBaritone().getMineProcess(), java.util.List.of(blocks));
    }

    @Override
    public void follow(Predicate<Entity> entity) {
        BaritoneAPI.getProvider().getPrimaryBaritone().getFollowProcess().follow((Predicate) entity);
    }
