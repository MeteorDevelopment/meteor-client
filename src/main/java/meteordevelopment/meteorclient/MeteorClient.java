...
public class MeteorClient implements ClientModInitializer {
    ...
    private void onTick(TickEvent.Post event) {
        if (mc.screen instanceof WidgetScreen) {
            // On 26.1 the old WidgetScreen renderer still piggybacks on the vanilla GUI pass,
            // so hiding the HUD here can make the entire Meteor GUI disappear.
            mc.options.hideGui = false;
        }

        // Recover from widget-screen close paths that fail to restore HUD visibility on 26.1.
        if (!(mc.screen instanceof WidgetScreen) && wasWidgetScreen) {
            mc.options.hideGui = wasHudHiddenRoot;
            wasWidgetScreen = false;
        }
    ...
    private void toggleGui() {
        if (Utils.canCloseGui()) mc.screen.onClose();
            ...
            // The old saved layout can place Meteor windows outside the visible area on 26.1.
            // Reset once on open so the original GUI can recover instead of opening invisibly.
