...
public class SaveMapCommand extends Command {
    ...
    private void saveMap(int scale) throws CommandSyntaxException {
        ...
        MapTextureManager textureManager = mc.getMapTextureManager();
        var textureId = textureManager.prepareMapTexture(map.get(DataComponents.MAP_ID), state);
        if (!(mc.getTextureManager().getTexture(textureId) instanceof DynamicTexture texture) || texture.getPixels() == null) throw OOPS.create();

        try {
            if (scale == 128) texture.getPixels().writeToFile(path);
            else {
                int[] data = texture.getPixels().makePixelArray();
