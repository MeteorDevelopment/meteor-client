...
public class PacketMine extends Module {
    ...
    public class MyBlock {
        ...
        private void sendMinePackets() {
            if (timer <= 0) {
                if (!mining) {
                    ((ClientPlayerInteractionManagerAccessor) mc.gameMode).meteor$startPrediction(mc.level, (sequence) -> new ServerboundPlayerActionPacket(ServerboundPlayerActionPacket.Action.START_DESTROY_BLOCK, blockPos, direction, sequence));
                    ((ClientPlayerInteractionManagerAccessor) mc.gameMode).meteor$startPrediction(mc.level, (sequence) -> new ServerboundPlayerActionPacket(ServerboundPlayerActionPacket.Action.STOP_DESTROY_BLOCK, blockPos, direction, sequence));
