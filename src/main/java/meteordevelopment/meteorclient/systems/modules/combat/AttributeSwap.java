...
public class AttributeSwap extends Module {
    ...
    private int getSmartSlot(Entity target) {
        ...
        boolean isUndead = target != null && target.getType().builtInRegistryHolder().is(EntityTypeTags.SENSITIVE_TO_SMITE);
        boolean isArthropod = target != null && target.getType().builtInRegistryHolder().is(EntityTypeTags.SENSITIVE_TO_BANE_OF_ARTHROPODS);
        boolean isAquatic = target != null && target.getType().builtInRegistryHolder().is(EntityTypeTags.SENSITIVE_TO_IMPALING);
