...
public class SwarmHost extends Thread {
    ...
    public SwarmHost(int port) {
        try {
            ...
            socket = null;
            ChatUtils.errorP("Swarm", "Couldn't start a server on port %s.", port);
            e.printStackTrace();
        }
    ...
    @Override
    public void run() {
        ChatUtils.infoP("Swarm", "Listening for incoming connections on port %s.", socket.getLocalPort());
        ...
        while (!isInterrupted()) {
            try {
                ...
                ChatUtils.errorP("Swarm", "Error making a connection to worker.");
                e.printStackTrace();
            }
        }
    }
    ...
    public void disconnect() {
        ...
        ChatUtils.infoP("Swarm", "Server closed on port %s.", socket.getLocalPort());
