...
public class Bounce extends ElytraFlightMode {
    ...
    public void onTick() {
        ...
        if (mc.options.keyJump.isDown() && !mc.player.isFallFlying() && !elytraFly.manualTakeoff.get()) mc.getConnection().send(new ServerboundPlayerCommandPacket(mc.player, ServerboundPlayerCommandPacket.Action.START_FALL_FLYING));

        // Make sure all the conditions are met (player has an elytra, isn't in water, etc)
        if (checkConditions(mc.player)) {
            if (!rubberbanded) {
                if (prevFov != 0 && !elytraFly.sprint.get()) mc.options.fovEffectScale().set(0.0); // This stops the FOV effects from constantly going on and off.
