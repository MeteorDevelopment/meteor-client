...
public class OutlineRenderCommandQueue extends SubmitNodeStorage {
    ...
    public SubmitNodeCollection order(int i) {
        return getSubmitsPerOrder().computeIfAbsent(i, order -> new OutlineBatchingRenderCommandQueue(this));
    }
    ...
    private class OutlineBatchingRenderCommandQueue extends SubmitNodeCollection {
        ...
        @Override
        public void submitBlockModel(PoseStack matrices, RenderType renderLayer, List<BlockStateModelPart> modelParts, int[] colors, int light, int overlay, int outlineColor) {
            int[] outlineColors = new int[colors.length];
            Arrays.fill(outlineColors, color);
            super.submitBlockModel(matrices, renderLayer, modelParts, outlineColors, light, overlay, 0);
        }

        @Override
        public void submitItem(PoseStack matrices, ItemDisplayContext displayContext, int light, int overlay, int outlineColors, int[] tintLayers, List<BakedQuad> quads, ItemStackRenderState.FoilType glintType) {
            if (tints == null || tints[0] != color) {
                tints = new int[] { color, color, color, color };
            }

            super.submitItem(matrices, displayContext, light, overlay, outlineColors, tints, quads, glintType);
        }
