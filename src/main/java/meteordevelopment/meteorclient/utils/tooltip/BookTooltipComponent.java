...
public class BookTooltipComponent implements ClientTooltipComponent, MeteorTooltipData {
    ...
    public void extractImage(Font textRenderer, int x, int y, int width, int height, GuiGraphicsExtractor context) {
        // Background
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE_BOOK_BACKGROUND, x - 10, y, 0f, 0f, 128, 128, 179, 179);
        ...
        for (FormattedCharSequence line : textRenderer.split(page, 112)) {
            context.text(textRenderer, line, 0, offset, 0xFF000000, false);
            offset += 8;
        }
        matrices.popMatrix();
    }
