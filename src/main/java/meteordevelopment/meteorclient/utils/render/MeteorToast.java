...
public class MeteorToast implements Toast {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, Font textRenderer, long startTime) {
        ...
        if (text != null) {
            context.text(textRenderer, text, textX, 18, TEXT_COLOR, false);
            titleY = 7;
        }

        context.text(textRenderer, title, textX, titleY, TITLE_COLOR, false);

        if (icon != null) context.item(icon, 8, 8);
    }
}
