...
public class EnchantmentListSetting extends Setting<Set<ResourceKey<Enchantment>>> {
    ...
    public Iterable<Identifier> getIdentifierSuggestions() {
        return Optional.ofNullable(Minecraft.getInstance().getConnection())
            .stream()
            .flatMap(networkHandler -> networkHandler.registryAccess().lookup(Registries.ENCHANTMENT).stream())
            .flatMap(registry -> registry.listElementIds().map(ResourceKey::identifier))
