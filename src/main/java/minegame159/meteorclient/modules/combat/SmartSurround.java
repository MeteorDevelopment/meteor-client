...
public class SmartSurround extends ToggleModule {
    ...
    private final Listener<EntityAddedEvent> onSpawn = new Listener<>(event -> {
        ...
        if(event.entity.getType() == EntityType.END_CRYSTAL){
            if(DamageCalcUtils.resistanceReduction(mc.player, DamageCalcUtils.blastProtReduction(mc.player, DamageCalcUtils.armourCalc(mc.player, DamageCalcUtils.getDamageMultiplied(DamageCalcUtils.crystalDamage(mc.player, event.entity))))) > minDamage.get()){
