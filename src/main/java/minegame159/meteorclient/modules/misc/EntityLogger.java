...
public class EntityLogger extends ToggleModule {
    ...
    private final Listener<EntityAddedEvent> onEntityAdded = new Listener<>(event -> {
        if (entities.get().contains(event.entity.getType())) {
            Utils.sendMessage(String.format("#blue[Meteor]: #white%s #grayspawned at #white%.0f#gray, #white%.0f#gray, #white%.0f#gray.", event.entity.getType().getName().asString(), event.entity.x, event.entity.y, event.entity.z));
        }
    });
}
