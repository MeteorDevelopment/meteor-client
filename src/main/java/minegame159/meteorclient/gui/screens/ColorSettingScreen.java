...
public class ColorSettingScreen extends WindowScreen {
    public interface Action {
        public void onAction(ColorSettingScreen colorSettingScreen);
    }
    ...
    private static final double ROOT3 = 1.73205080757;
    ...
    public ColorSettingScreen(Setting<Color> setting) {
        ...
        rItb = rgbaTable.add(new WIntEdit(0, 255, setting.get().r)).getWidget();
        rItb.action = intEdit -> rgbaChanged();
        rgbaTable.row();

        rgbaTable.add(new WLabel("G:"));
        gItb = rgbaTable.add(new WIntEdit(0, 255, setting.get().g)).getWidget();
        gItb.action = intEdit -> rgbaChanged();
        rgbaTable.row();

        rgbaTable.add(new WLabel("B:"));
        bItb = rgbaTable.add(new WIntEdit(0, 255, setting.get().b)).getWidget();
        bItb.action = intEdit -> rgbaChanged();
        rgbaTable.row();

        rgbaTable.add(new WLabel("A:"));
        aItb = rgbaTable.add(new WIntEdit(0, 255, setting.get().a)).getWidget();
        aItb.action = intEdit -> rgbaChanged();
        ...
        backButton.action = button -> onClose();

        WButton resetButton = bottomTable.add(new WButton(GuiRenderer.TEX_RESET)).getWidget();
    ...
    private class WHueQuad extends WWidget {
        ...
        protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            ...
            for (int i = 0; i < HUE_COLORS.length - 1; i++) {
                renderer.renderQuad(sectionX, y, sectionWidth, height, HUE_COLORS[i], HUE_COLORS[i + 1]);
